    ...
    <div class="card card-custom card-custom gutter-t">
        <div class="card-header py-3">
            ...
            <div class="card-toolbar">
                <a href="{{ route('colors.create') }}" class="btn btn-success btn-sm">Add Color</a>
            </div>
        </div>
        <div class="card-body">
                ...
                <tbody id="tablecontents">
                    @foreach ($colors as $color)
                        <tr class="row1" data-id="{{ $color->id }}">
                            ...
                            <td>
                                <a href="{{ route('colors.edit', $color->id) }}" class="delete btn btn-primary btn-sm">Edit</a>
                                <form style="display: inline-block" action="{{ route('colors.destroy', $color->id) }}" method="POST">
                                    @csrf
                                    @method('delete')
